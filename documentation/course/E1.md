...
- `LOG/01` - Jako _administrator_ chcę mieć możliwość przeglądania, dodawania, edycji i usuwania _linii_, aby zarządzać trasami _pojazdów_.
- `LOG/02` - Jako _administrator_ chcę mieć możliwość przeglądania, dodawania, edycji i usuwania _przystanków_, aby zarządzać miejscami, w których _pasażerowie_ mogą wsiąść lub wysiąść z _pojazdów_.
- `LOG/03` - Jako _administrator_ chcę mieć możliwość przeglądania, dodawania, edycji i usuwania _rozkładów jazdy_, aby zarządzać godzinami odjazdów _pojazdów_ z _przystanków_.
- `LOG/04` - Jako _administrator_ chcę mieć możliwość przeglądania, dodawania, edycji i usuwania _pojazdów_, aby zarządzać dostępnymi środkami transportu.
- `LOG/05` - Jako _kierowca_ chcę mieć możliwość przeglądania, zgłaszania i edycji _awarii_, aby informować _pasażerów_ o problemach na _kursie_.
- `LOG/06` - Jako _bileter_ chcę mieć możliwość przeglądania, zgłaszania i edycji _awarii_, aby informować _pasażerów_ o problemach na _kursie_.
- `LOG/07` - Jako _administrator_ chce mieć możliwość przeglądania _awarii_, aby monitorować sytuację na _trasach_.
...
classDiagram
    ...
    class Stop {
        long id [1] #lcub;id, readonly#rcub;
        String name [1] #lcub;readonly#rcub;
        BigDecimal latitude [1] #lcub;readonly#rcub;
        BigDecimal longitude [1] #lcub;readonly#rcub;
    }
...
- `REG/14/07` - Przystanek musi zawierać szerokość geograficzną.
- `REG/14/08` - Szerokość geograficzna musi być poprawną wartością.
- `REG/14/09` - Przystanek musi zawierać długość geograficzną.
- `REG/14/10` - Długość geograficzna musi być poprawną wartością.
...
classDiagram
    direction LR
    class Logowanie { <<serverPage>> }
    class MenuAdministratora { <<serverPage>> }
    class ZarzadzaniePrzystankami { <<serverPage>> }
    class EdycjaPrzystanku { <<modalPopup>> }
    class UsuwaniePrzystanku { <<modalPopup>> }

    Logowanie --> MenuAdministratora
    MenuAdministratora --> ZarzadzaniePrzystankami : btnPrzystanki
    ZarzadzaniePrzystankami --> EdycjaPrzystanku : btnEdytuj
    EdycjaPrzystanku --> ZarzadzaniePrzystankami : btnZapisz
    EdycjaPrzystanku --> ZarzadzaniePrzystankami : btnAnuluj

    ZarzadzaniePrzystankami --> UsuwaniePrzystanku : btnUsuń
    UsuwaniePrzystanku --> ZarzadzaniePrzystankami : btnTak
    UsuwaniePrzystanku --> ZarzadzaniePrzystankami : btnAnuluj

    EdycjaPrzystanku --> UsuwaniePrzystanku : btnUsuń
    UsuwaniePrzystanku --> EdycjaPrzystanku : btnAnuluj
    UsuwaniePrzystanku --> ZarzadzaniePrzystankami : btnTak
```

#### Prototypy ekranów

- `LOG/02/01` - Przeglądanie listy przystanków.

![LOG/02/01](images/Stop1.png)

- `LOG/02/02` - Dodawanie przystanku.

![LOG/02/02](images/Stop2.png)

- `LOG/02/03` - Edycja przystanku.

![LOG/02/03](images/Stop3.png)

- `LOG/02/04` - Usuwanie przystanku.

![LOG/02/04](images/Stop4.png)
...
classDiagram
    direction LR
    class Logowanie { <<screen>> }
    class MenuBiletera { <<screen>> }
    class ListaAwarii { <<screen>> }
    class EdycjaAwarii { <<popup>> }
    class DodawanieAwarii { <<popup>> }

    Logowanie --> MenuBiletera
    MenuBiletera --> ListaAwarii : btnAwarie
    ListaAwarii --> EdycjaAwarii : btnEdytuj
    EdycjaAwarii --> ListaAwarii : btnZatwierdz
    EdycjaAwarii --> ListaAwarii : btnAnuluj

    ListaAwarii --> DodawanieAwarii : btnDodaj
    DodawanieAwarii --> ListaAwarii : btnZatwierdz
    DodawanieAwarii --> ListaAwarii : btnAnuluj
```

#### Prototypy ekranów

- `LOG/06/01` - Przeglądanie listy awarii.

![LOG/06/01-01](images/Accident4.png)
![LOG/06/01-02](images/Accident3.png)

- `LOG/06/02` - Zgłaszanie awarii.

![LOG/06/02](images/Accident2.png)

- `LOG/06/03` - Edycja awarii.

![LOG/06/03](images/Accident1.png)
