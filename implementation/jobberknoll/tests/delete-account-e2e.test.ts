...
Deno.test(
  "DELETE /ext/v1/accounts/{id} should return user-unauthorized if user is not an admin",
  async () => {
    const { api, accountRepo } = setupTest();
    await accountRepo.createAccount(accountMock);
...
Deno.test(
  "DELETE /ext/v1/accounts/{id} should return schema-mismatch if the account id is not an UUID",
  async () => {
    const { api } = setupTest();
...
Deno.test(
  "DELETE /ext/v1/accounts/{id} should return account-not-found if the account does not exist",
  async () => {
    const { api } = setupTest();
...
Deno.test(
  "DELETE /ext/v1/accounts/{id} should make the account unfetchable from GET /ext/v1/accounts/{id}",
  async () => {
    const { api, accountRepo } = setupTest();
    await accountRepo.createAccount(accountMock);

    await api.request(`/ext/v1/accounts/${accountMock.id}`, {
      method: "DELETE",
      headers: correctHeaders,
    });
