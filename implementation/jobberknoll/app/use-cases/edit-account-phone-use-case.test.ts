...
Deno.test("EditAccountPhoneUseCase should return an ok result if the account exists", async () => {
  ...
  const result = await editAccountPhone.invoke(newCtx(), { accountId: accountMock.id, phoneNumber: "123 456 789" });
...
Deno.test("EditAccountPhoneUseCase should audit the phone change if the account exists", async () => {
  ...
  await editAccountPhone.invoke(newCtx(), { accountId: accountMock.id, phoneNumber: "123 456 789" });
...
Deno.test("EditAccountPhoneUseCase should throw if the account is not a passenger", async () => {
  ...
  assertRejects(() => editAccountPhone.invoke(newCtx(), { accountId: accountMock.id, phoneNumber: "123 456 789" }));
});
